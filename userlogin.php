...
<nav>
    ...
    <h1>Welcome to Self Publication</h1>
    <div class="container">
        <div class="tab">
            <button class="tablink active" onclick="openTab(event,'signin')" id="link1" style="background-color: #d9534f;">Login</button>
        ...
        <div class="content">
            <div class="tabcontent" id="signin">
                <form method="post" onsubmit="return validateLogin()">
                    ...
                    <div class="required"></div>
                    <br><br>
                    <input type="submit" value="Login" name="login"  style="background-color: #d9534f;border-radius: 8px;">
                </form>
            </div>
            <div class="tabcontent" id="signup">
                <form method="post" onsubmit="return validateRegister()">
                    ...
                    <br><br>
                    <input type="submit" value="Create Account" name="register" style="background-color: #d9534f; border-radius: 8px;">
                </form>
            </div>
        </div>
    </div>
    <script src="resources/js/main.js"></script><br><br><br><br><br><br>
    <nav>
    <div class="nav-wrapper">
      
    <div class="footer-copyright">
            <div class="container">
            ©Copyright www.fastag.org 2020
            <a class="grey-text text-lighten-4 right" href="http://www.fastag.org/">More Links</a>
            </div>
          </div>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // A form is posted
    if (isset($_POST['login'])) { // Login process
        ...
        $query = mysqli_query($conn, "SELECT * FROM userslogin WHERE user_email = '$useremail' AND user_password = '$userpass'");
        if($query){
    ...
    if (isset($_POST['register'])) { // Register process
        ...
        $query = mysqli_query($conn, "SELECT user_nickname, user_email FROM userslogin WHERE user_nickname = '$usernickname' OR user_email = '$useremail'");
        ...
        // Insert Data
        $sql = "INSERT INTO userslogin(user_firstname, user_lastname, user_nickname, user_password, user_email, user_gender, user_birthdate, user_status, user_about, user_hometown)
        ...
        if($query){
            $query = mysqli_query($conn, "SELECT user_id FROM userslogin WHERE user_email = '$useremail'");
            $row = mysqli_fetch_assoc($query);
            $_SESSION['user_id'] = $row['user_id'];
            header("location:userhome.php");
        }
